...
export async function inspectTarget(adminUrl: string, database: string): Promise<TargetInspection> {
  ...
  try {
    ...
    const hasMarker = marker.rows[0]?.present === true;
    ...
    if (hasMarker) {
      const row = await client.query<{ label: string; created_at: Date | string }>(
        `SELECT label, created_at FROM ${REHEARSAL_MARKER_TABLE} ORDER BY created_at LIMIT 1`,
      );
      label = row.rows[0]?.label ?? null;
      createdAt =
        row.rows[0] === undefined ? null : new Date(row.rows[0].created_at).toISOString();
    }
    return {
      exists: true,
...
export function assertSelectableTarget(inspection: TargetInspection, database: string): void {
  ...
  if (inspection.has_marker) return;
